import { SearchStore, SearchTypeEnum } from "./SearchStore";
...
describe("Search Class", () => {
	let searchInstance: SearchStore;
	beforeEach(() => {
		searchInstance = new SearchStore();
	});
	...
	it("initializes with default values", () => {
		...
		expect(searchInstance.searchType).toBe(SearchTypeEnum.repositories);
	...
	it("sets the search type and resets the state", () => {
		...
		expect(searchInstance.searchType).toBe(SearchTypeEnum.users);
		expect(searchInstance.query).toBe("");
		expect(searchInstance.page).toBe(1);
		expect(searchInstance.itemsNumber).toBe(0);
		expect(searchInstance.validation).toEqual({
	...
	it("handles error correctly in the catch block", async () => {
		...
		// Set initial state
		const searchInstance = new SearchStore();
		...
		expect(searchInstance.itemsNumber).toBe(0);
		expect(searchInstance.validation.isValid).toBe(false);
		expect(searchInstance.validation.errorMessage).toBe("octokit.rest.search[this.searchType] is not a function");
	});
