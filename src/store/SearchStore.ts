...
export class SearchStore {
	...
	public searchType: SearchTypeEnum = SearchTypeEnum.repositories;
	...
	public async searchGitHub() {
		try {
            this.toggleLoading();
			const res = await octokit.rest.search[this.searchType]({
				q: this.formatQuery(this.query),
				page: this.page,
				per_page: 10,
			});

			if (this.searchType === SearchTypeEnum.repositories) {
				this.setRepoSearchResult(res.data as SearchReposType);
			} else if (this.searchType === SearchTypeEnum.users) {
